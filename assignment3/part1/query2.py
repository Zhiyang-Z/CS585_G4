...
distance_RDD = (
    ...
    .map(lambda x: x[0][0]) # map to id1
    .distinct() # get distinct id1s
)

distance_RDD_data = distance_RDD.collect()
for record in distance_RDD_data[0:5]:
    print(record)
print(f"Total pairs with distance less than 6: {len(distance_RDD_data)}")
